<!--轮播图 https://github.com/derickweng/vue-banner -->
<template>
    <div class="banner">
        <div class="slider-wrap">
            <ul class="slider-items" :style="[sliderActive,{'width':sliderImg.length*100+'vw'}]" @touchstart="stopSlider($event)" @touchmove="moveSlider($event)" @touchend="continSilder($event)">
                <li class="slider-item" v-for="(item,index) in sliderImg" ><img :src="getURl(item,index)" class="item-img"></li>
            </ul>
        </div>
...
export default {
    ...
    methods : {
        getURl (item,index) {  //父组件传list就用list  没有 就用自带的图
            if (!item) {
                return reloadImg
            }
            if (item) {
